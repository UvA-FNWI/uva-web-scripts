"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
var Language_1 = require("./Language");
var BrowserWarning = /** @class */ (function () {
    function BrowserWarning() {
    }
    // Warning text for an IE browser of given version.
    BrowserWarning.IEWarningText = function (version) {
        var warning = "";
        if (Language_1.Language.isEnglish()) {
            if (version === 11) {
                warning += "Your browser is not well supported. ";
                warning += "Continue at your own risk. ";
            }
            else {
                warning += "Your browser is NOT supported. ";
            }
            warning += " We suggest using Chrome, Edge or Firefox.";
        }
        else {
            if (version === 11) {
                warning += "Uw browser wordt niet goed ondersteund. ";
                warning += "Ga door op eigen risico. ";
            }
            else {
                warning += "Uw browser wordt NIET ondersteund. ";
            }
            warning += "We raden aan Chrome, Edge of Firefox te gebruiken.";
        }
        return warning;
    };
    BrowserWarning.SafariWarningText = function (version) {
        var warning = '';
        if (Language_1.Language.isEnglish()) {
            warning += "Your browser is not well supported. ";
            warning += " We suggest upgrading your browser to the latest version. ";
            warning += " If you encounter any issues with login, please try a different browser.";
        }
        else {
            warning += "Uw browser wordt niet goed ondersteund. ";
            warning += " We raden u aan uw browser te upgraden naar de nieuwste versie. ";
            warning += "Als u problemen ondervindt met inloggen, probeer een andere browser.";
        }
        return warning;
    };
    BrowserWarning.WarnOldBrowser = function () {
        // Show a warning only on IE.
        if (navigator.browserSpecs.name === "IE") {
            // Wait until the unsupported browser dialog has loaded.
            var intervalId = setInterval(function () {
                var dialogs = $$('.unsupported-browser-dialog');
                if (dialogs.length > 0) {
                    clearInterval(intervalId);
                    $$('.unsupported-browser-dialog-text')[0]
                        .innerHTML = BrowserWarning.IEWarningText(parseInt(navigator.browserSpecs.version));
                    dialogs.toggle();
                }
            }, 100);
        }
        // warining for safari, login issues
        if (navigator.browserSpecs.name.toLowerCase() === "safari" && navigator.browserSpecs.version < 12) {
            var intervalId = setInterval(function () {
                var dialogs = $$('.unsupported-browser-dialog');
                if (dialogs.length > 0) {
                    clearInterval(intervalId);
                    $$('.unsupported-browser-dialog-text')[0]
                        .innerHTML = BrowserWarning.SafariWarningText(parseInt(navigator.browserSpecs.version));
                    dialogs.toggle();
                }
            }, 100);
        }
    };
    return BrowserWarning;
}());
exports.BrowserWarning = BrowserWarning;
// Determine browser name and version and attach to navigator.browserSpecs.
// From: https://stackoverflow.com/a/38080051
navigator.browserSpecs = (function () {
    var ua = navigator.userAgent, tem, M = ua.match(/(opera|chrome|safari|firefox|msie|trident(?=\/))\/?\s*(\d+)/i) || [];
    if (/trident/i.test(M[1])) {
        tem = /\brv[ :]+(\d+)/g.exec(ua) || [];
        return { name: 'IE', version: (tem[1] || '') };
    }
    if (M[1] === 'Chrome') {
        tem = ua.match(/\b(OPR|Edge)\/(\d+)/);
        if (tem != null)
            return { name: tem[1].replace('OPR', 'Opera'), version: tem[2] };
    }
    M = M[2] ? [M[1], M[2]] : [navigator.appName, navigator.appVersion, '-?'];
    if ((tem = ua.match(/version\/(\d+)/i)) != null)
        M.splice(1, 1, tem[1]);
    return { name: M[0], version: M[1] };
})();
// When the browser is ready show a browser unsupported warning if necessary.
jQuery(document).ready(BrowserWarning.WarnOldBrowser);
