...
class OrderTable {
    ...
    static makeTableOrderable(id, lockedRowCount) {
        var drag = {
            dragging: false,
            el: null,
            body: null,
            ...
            movers[i].onmousedown = (function (mover, i) {
                return function (event) {
                    ...
                    if (!placeHolder.childNodes.length) {
                        ...
                        for (var k = 0; k < tdCount; ++k) { placeHolder.appendChild(document.createElement('td')); }
                    }
        ...
        var mousemove = function (event) {
            if (drag.dragging) { orderPlaceHolder(event.clientY); }
        };
        ...
        var mouseup = function (event) {
            if (drag.dragging) {
                ...
                if (drag.index != index) {
                    DN.Event(id, 'OrderChange', { iOld: drag.index, iNew: index }, false);
                }
            }
            drag.dragging = false;
            drag.el = null;
