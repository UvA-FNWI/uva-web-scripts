...
function HeaderCellCopy(original, width, height, rightPadding) {
    if (rightPadding === void 0) { rightPadding = ""; }
    var headerCell = document.createElement("div");
    headerCell.className = "floating-header no-border-bottom";
    headerCell.style.width = width;
    headerCell.style.height = height;
    headerCell.style.paddingRight = rightPadding;
...
function HeaderCopy(headerCells) {
    ...
    for (var i = 0; i < headerCells.length; i++) {
        ...
        header.appendChild(HeaderCellCopy(headerCells[i], style.getPropertyValue("width"), style.getPropertyValue("height"), style.getPropertyValue("padding-right")));
        if (i == 0)
...
var Grid = /** @class */ (function () {
    ...
    Grid.HideFloatingHeaders = function (except) {
        var ids = Object.keys(Grid.FloatingHeaderHidden);
